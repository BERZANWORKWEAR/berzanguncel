...
function buildDefaultDb() {
  ...
  if (!Array.isArray(db.workflows)) {
    const createdAt = nowIso();
    db.workflows = [
      {
        id: "wrk_seed_1",
        title: "Demir Lojistik numune takibi",
        customer_id: "cus_berzan_demo_1",
        lead_id: "lead_demo_1",
        order_id: "ord_demo_1",
        ...
        next_action: "Numune setini gönder ve onay al",
        next_action_date: plusDays(1),
        owner: "Satış Ekibi",
        notes: "Demo seed iş takibi.",
...
function ensureDbShape(db) {
  ...
  db.workflows = Array.isArray(db.workflows) ? db.workflows : fallback.workflows || [];
  ...
  return mutateDb((db) => {
    ...
    db.workflows.unshift(
      sanitizeWorkflow({
        ...
        next_action: "Termin ve teslim planını netleştir",
        next_action_date: plusDays(2),
        owner: db.settings.salesOwner || "Satış Ekibi",
        notes: "Teklif yerel modda siparişe çevrildi.",
      })
    );
